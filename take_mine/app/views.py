...
from .models import Category, ExchangeProposal, Item
from .permissions import can_edit_item, cannot_exchange_own_item
from .utils import get_page_context


@cache_page(60 * 15)
def index(request):
    """Главная страница со списком доступных для обмена предметов.

    Поддерживает фильтрацию по:
    - поисковому запросу (в заголовке и описании)
    - категории
    - состоянию предмета"""
    ...
    exchanged_items = ExchangeProposal.objects.filter(status="принята").values_list(
...
def item_create(request):
    """
    Создание нового объявления о предмете для обмена.

    Доступен только авторизованным пользователям.
    При успешном создании перенаправляет на главную страницу.
    """
...
def create_category(request):
    ...
    context = {"form": form, "title": "Создание новой категории"}
...
def item_edit(request, item_id):
    """
    Редактирование существующего объявления о предмете.

    Доступен только владельцу предмета.
    При успешном редактировании перенаправляет в профиль.
    """
...
def create_exchange_proposal(request, item_id):
    """
    Создание предложения обмена для выбранного предмета.

    Проверяет права пользователя и возможность обмена.
    При успешном создании перенаправляет на список обменов.
    """
    ...
    return render(
        request,
        "app/exchange_proposal.html",
        {"receiver_item": receiver_item, "items": items},
    )
...
def exchanges_list(request):
    """
    Список предложений обмена для текущего пользователя.

    Поддерживает фильтрацию по:
    - типу предложений (входящие/исходящие)
    - статусу предложения
    """
