...
class ExchangeProposalSerializer(serializers.ModelSerializer):
    ad_sender = serializers.SlugRelatedField(
        queryset=Item.objects.all(), slug_field="title", write_only=True
    )
...
class MyProposalsSerializer(serializers.ModelSerializer):
    ...
    ad_receiver_title = serializers.CharField(
        source="ad_receiver.title", read_only=True
    )
    ad_sender_user = serializers.CharField(
        source="ad_sender.user.username", read_only=True
    )
    ad_receiver_user = serializers.CharField(
        source="ad_receiver.user.username", read_only=True
    )
